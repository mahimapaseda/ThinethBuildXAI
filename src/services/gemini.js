...
- Seismic zone (per IS 1893 if in India/South Asia)
...
**Your Task:**
Provide a professional Civil Engineering assessment following international standards (IS 456, ACI 318, Eurocodes). Explain everything in extreme detail as the user needs a comprehensive, massive report. Explain it so a person with ZERO construction knowledge can understand.
...

{
  ...
  "materialEstimateSummary": {
    ...
    "steelTons": 0,
    "bricksBlocks": 0,
    "currentMarketRateNotes": "Estimation of total cost based on common current market prices"
  },
  "stepByStepGuide": [
    {
...
CRITICAL JSON RULES (MUST FOLLOW OR SYSTEM WILL CRASH):
1. Write extremely long, detailed paragraphs for the text fields to provide a massive report.
2. DO NOT use raw newlines (Enter key) inside string values. If you need a paragraph break, explicitly type the characters \\n.
3. Escape any double quotes inside your text using \\".
4. Output ONLY the raw JSON object. Do NOT wrap it in markdown \`\`\`json blocks.
5. Ensure the JSON object is completely valid and properly closed at the very end.
6. All calculations must be grounded in engineering reality based on the building size.`;
  ...
  try {
    ...
    try {
      ...
      if (codeBlockMatch) {
        parsed = JSON.parse(sanitizeJsonString(codeBlockMatch[1].trim()));
      } else {
        // Strategy 3: Find the first { and last } to extract the JSON object
        const firstBrace = text.indexOf('{');
        const lastBrace = text.lastIndexOf('}');
        if (firstBrace !== -1 && lastBrace > firstBrace) {
          parsed = JSON.parse(sanitizeJsonString(text.substring(firstBrace, lastBrace + 1)));
        } else {
      ...
      try {
        ...
        try {
          ...
          if (si === -1 || ei <= si) throw new Error('No JSON braces found');
          let candidate = raw.substring(si, ei + 1);
